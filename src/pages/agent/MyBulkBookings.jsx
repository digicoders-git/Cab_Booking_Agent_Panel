...
const generateReceipt = async (booking) => {
  try {
    ...
    const blob = await agentService.downloadBulkReceipt(booking._id);
    const url = window.URL.createObjectURL(new Blob([blob], { type: 'application/pdf' }));
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `KwikCabs_Receipt_${booking._id.toString().slice(-6).toUpperCase()}.pdf`);
